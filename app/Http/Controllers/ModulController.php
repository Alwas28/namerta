...
class ModulController extends Controller
{
    ...
    public function show($courseId, $modulId)
    {
        ...
        if ($jenisTestKompetensi) {
            if ($jenisTestKompetensi->essay == 'N') {
                // Pilihan Ganda
                $soalPilgan = SoalTesKompetensi::where('id_modul', $modulId)->get();
                if ($userId) { // jika siswa
        ...
        return view('mata_pelajaran.modul_show', compact(
            ...
            'koneksiMateriFile',
            'aksiNyataAnswer',
            'modul', 'materials', 'checklist', 'courseId', 'canManage',
    ...
    /**
     * Store a newly created module (Teacher/Admin only)
     */
    public function store(Request $request, $courseId)
    {
        if (!$this->hasManageAccess($courseId)) {
            return redirect()->route('courses.index')
                ->with('error', 'Anda tidak memiliki akses untuk mengelola modul ini');
        }
        
        // Get course info to get mata_pelajaran_id
        $course = DB::table('kelas_mp')
            ->join('mata_pelajaran', 'kelas_mp.id_mata_pelajaran', '=', 'mata_pelajaran.id_mata_pelajaran')
            ->where('kelas_mp.id_kelas_mp', $courseId)
            ->select('kelas_mp.*', 'mata_pelajaran.id_mata_pelajaran')
            ->first();
        ...
        $request->validate([
            ...
            'koneksi_materi' => 'nullable|string', 
            'tes_kompetensi' => 'nullable|string',
            'aksi_nyata' => 'nullable|string'
        ]);
        
        try {
            DB::beginTransaction();
            
            $modulId = DB::table('modul')->insertGetId([
                'nama_modul' => $request->nama_modul,
                'desk' => $request->desk,
                'pengalaman_belajar' => $request->pengalaman_belajar,
                'koneksi_materi' => $request->koneksi_materi,
                'tes_kompetensi' => $request->tes_kompetensi,
                'aksi_nyata' => $request->aksi_nyata,
                'id_mata_pelajaran' => $course->id_mata_pelajaran,
                'created_at' => now(),
                'updated_at' => now()
            ]);
            ...
            return redirect()->route('modul.index', $courseId)
            ...
            return redirect()->back()
                ->with('error', 'Terjadi kesalahan saat menambahkan modul. Silakan coba lagi.')
                ->withInput();
        }
    }

    /**
     * Remove the specified module (Teacher/Admin only)
     */
    public function destroy($courseId, $modulId)
    {
        if (!$this->hasManageAccess($courseId)) {
            return redirect()->route('courses.index')
                ->with('error', 'Anda tidak memiliki akses untuk mengelola modul ini');
        ...
        try {
            \Log::info('Attempting to delete module', ['module_id' => $modulId, 'course_id' => $courseId]);
            ...
            // Get all materi IDs that belong to this module
            $materiIds = DB::select('SELECT id_materi FROM materi WHERE id_modul = ?', [$modulId]);
            $materiIdArray = array_column($materiIds, 'id_materi');
            
            \Log::info('Found materi IDs', ['ids' => $materiIdArray]);
            
            $deletedRows = [];
            
            if (!empty($materiIdArray)) {
                $placeholders = implode(',', array_fill(0, count($materiIdArray), '?'));
                
                // Get all eksplorasi_konsep IDs that belong to these materials
                $eksplorasiKonsepIds = DB::select("SELECT id_eksplorasi_konsep FROM eksplorasi_konsep WHERE id_materi IN ($placeholders)", $materiIdArray);
                ...
                // Delete records that reference eksplorasi_konsep
                if (!empty($eksplorasiKonsepIdArray)) {
                    $eksplorasiPlaceholders = implode(',', array_fill(0, count($eksplorasiKonsepIdArray), '?'));
                    
                    $deletedRows['jawaban_pengetahuan_metakognisi'] = DB::delete(
                        "DELETE FROM jawaban_pengetahuan_metakognisi WHERE id_eksplorasi_konsep IN ($eksplorasiPlaceholders)", 
                        $eksplorasiKonsepIdArray
                    );
                    
                    $deletedRows['soal_pengetahuan_metakognisi'] = DB::delete(
                        "DELETE FROM soal_pengetahuan_metakognisi WHERE id_eksplorasi_konsep IN ($eksplorasiPlaceholders)", 
                        $eksplorasiKonsepIdArray
                    );
                }
                
                // Delete records that reference materi directly
                $deletedRows['jawaban_mulai_dari_diri'] = DB::delete("DELETE FROM jawaban_mulai_dari_diri WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['jawaban_demonstrasi_konseptual'] = DB::delete("DELETE FROM jawaban_demonstrasi_konseptual WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['jawaban_elaborasi_pemahaman'] = DB::delete("DELETE FROM jawaban_elaborasi_pemahaman WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['jawaban_refleksi_terbimbing'] = DB::delete("DELETE FROM jawaban_refleksi_terbimbing WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['jawaban_ruang_kolaborasi'] = DB::delete("DELETE FROM jawaban_ruang_kolaborasi WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['jawaban_perencanaan_refleksi_evaluasi'] = DB::delete("DELETE FROM jawaban_perencanaan_refleksi_evaluasi WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['soal_perencanaan_refleksi_evaluasi'] = DB::delete("DELETE FROM soal_perencanaan_refleksi_evaluasi WHERE id_materi IN ($placeholders)", $materiIdArray);
                $deletedRows['checklist_materi'] = DB::delete("DELETE FROM checklist_materi WHERE id_materi IN ($placeholders)", $materiIdArray);
                
                // Delete eksplorasi_konsep records
                $deletedRows['eksplorasi_konsep'] = DB::delete("DELETE FROM eksplorasi_konsep WHERE id_materi IN ($placeholders)", $materiIdArray);
                
                // Delete materi records
                $deletedRows['materi'] = DB::delete("DELETE FROM materi WHERE id_modul = ?", [$modulId]);
            }
            
            // Delete records that reference modul directly
            $deletedRows['jawaban_aksi_nyata'] = DB::delete('DELETE FROM jawaban_aksi_nyata WHERE id_modul = ?', [$modulId]);
            $deletedRows['jawaban_koneksi_materi'] = DB::delete('DELETE FROM jawaban_koneksi_materi WHERE id_modul = ?', [$modulId]);
            $deletedRows['jawaban_essay_tes_kompetensi'] = DB::delete(
                'DELETE FROM jawaban_essay_tes_kompetensi WHERE id_soal_essay IN (SELECT id_soal_essay FROM soal_essay_tes_kompetensi WHERE id_modul = ?)', 
                [$modulId]
            );
            $deletedRows['jawaban_tes_kompetensi'] = DB::delete(
                'DELETE FROM jawaban_tes_kompetensi WHERE id_soal_tes_kompetensi IN (SELECT id_soal_tes_kompetensi FROM soal_tes_kompetensi WHERE id_modul = ?)', 
                [$modulId]
            );
            $deletedRows['soal_essay_tes_kompetensi'] = DB::delete('DELETE FROM soal_essay_tes_kompetensi WHERE id_modul = ?', [$modulId]);
            $deletedRows['soal_tes_kompetensi'] = DB::delete('DELETE FROM soal_tes_kompetensi WHERE id_modul = ?', [$modulId]);
            $deletedRows['jenis_tes_kompetensi'] = DB::delete('DELETE FROM jenis_tes_kompetensi WHERE id_modul = ?', [$modulId]);
            $deletedRows['checklist_modul'] = DB::delete('DELETE FROM checklist_modul WHERE id_modul = ?', [$modulId]);
            
            // Log what was deleted
            \Log::info('Deleted related records', $deletedRows);
            
            // Finally delete the module itself
            $deleted = DB::delete('DELETE FROM modul WHERE id_modul = ?', [$modulId]);
            ...
            if ($deleted) {
                DB::commit();
                \Log::info('Module deleted successfully', ['module_id' => $modulId]);
            ...
            return redirect()->back()
                ->with('error', 'Terjadi kesalahan saat menghapus modul. Silakan cek log untuk detail.');
        }
    }
    ...
    public function setJenisTes(Request $request, $courseId, $modulId)
    {
        if (!$this->hasManageAccess($courseId)) {
            return response()->json(['error' => 'Unauthorized'], 403);
        }
        ...
        $request->validate([
            'essay' => 'required|in:Y,N'
        ]);
        
        $existing = JenisTesKompetensi::where('id_modul', $modulId)->first();
        
        if ($existing) {
            $existing->update(['essay' => $request->essay]);
        } else {
            JenisTesKompetensi::create([
                'id_modul' => $modulId,
                'essay' => $request->essay
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Jenis tes berhasil diatur'
        ]);
    }
}